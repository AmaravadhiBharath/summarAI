...
export const saveHistoryToFirestore = async (userId: string, item: any) => {
    try {
        // Use item.id as document ID to prevent duplicates
        // Ensure we save to the correct path: users/{userId}/history/{itemId}
        const historyRef = doc(db, `users/${userId}/history/${item.id}`);

        // Save the full item, ensuring timestamp/date consistency
        await setDoc(historyRef, {
            ...item,
            timestamp: item.date || new Date().toISOString()
        });
        console.log("Saved history to Firestore:", item.id);
    } catch (error) {
